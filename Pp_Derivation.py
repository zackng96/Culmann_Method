...
rs.AddLine(S1,[ERSSBotRight+WeightX,-1*(ERSSHt+SoilHt1+SoilHt2),0])
...
PY = ERSSHt + SoilHt1 + SoilHt2 
PX = PY/math.tan(theta/180*math.pi)
rs.AddLine(S1,[ERSSBotRight+PX,PY,0])
PGradient = PY/PX
...
for coord in FArea:
    ...
    count += 1
rs.AddCurve(RcoordsList, degree = 2)
...
print("Pp is {0} kN/m".format(min(Distancelist)/Weight2Len))
